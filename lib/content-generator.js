...
const SHARED_CSS_PATH = path.join(PUBLIC_SITES_DIR, "_assets", "blog.css");
...
// Site files use relative links only, so each site works both under
// /sites/{slug}/ on the main deployment and at the root of its own
  ...
  const postCards = posts
    .map(
      (post) => `<article class="card">
        ...
        <a class="read-more" href="posts/${post.slug}.html">Read →</a>
      </article>`
    )
    .join("\n");
...
<body>
  <header class="hero compact">
    <p class="eyebrow"><a href="../">${escapeHtml(site.name)}</a></p>
    <h1>${escapeHtml(post.title)}</h1>
  </header>
  <main class="article">${post.body}</main>
  <footer class="footer">
    <p><a href="../">← All posts</a> · <a href="${baseUrl}/">StoreForge stream</a></p>
  </footer>
</body>
</html>`;
}
...
function writeSiteFiles(site, posts, baseUrl) {
  ...
  fs.copyFileSync(SHARED_CSS_PATH, path.join(assetsDir, "blog.css"));
