function renderDashboard(sites, baseUrl) {
  const cards = sites.length
    ? sites
        ...
        <h2><a href="${escapeHtml(site.url)}" target="_blank" rel="noopener">${escapeHtml(site.name)}</a></h2>
            <p>${escapeHtml(site.tagline)}</p>
            <div class="site-links">
              <a href="${escapeHtml(site.url)}" target="_blank" rel="noopener">Visit site →</a>
...
<head>
  ...
  <title>StoreForge · Live Blog Stream</title>
  <style>
    :root {
      color-scheme: dark;
      --bg: #0b1020;
...
<body>
  <div class="wrap">
    <header class="hero">
      <div>
        <p class="eyebrow">StoreForge Blog Forge</p>
    ...
    <p class="footer-note">
      ...
      Hourly workflow commits new sites to <code>public/sites/</code> and redeploys on Vercel.
    </p>
  </div>
</body>
</html>`;
